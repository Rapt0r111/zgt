...
import { ArrowLeft, Save, AlertCircle, History, HardDrive } from 'lucide-react';
...
export default function EquipmentDetailPage() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      setError(typeof detail === 'string' ? detail : 'Ошибка при обновлении');
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              ...
              {equipment.model && ` • ${equipment.manufacturer} ${equipment.model}`}
            </p>
          </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              {equipment.seal_status !== 'Исправна' && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    Пломба {equipment.seal_status.toLowerCase()}! Требуется проверка.
                  </AlertDescription>
                </Alert>
              )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      {isEditing ? (
                        ...
                        <div className="text-sm">{equipment.equipment_type}</div>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {isEditing ? (
                        ...
                        <div className="text-sm">{equipment.storage_type || '—'}</div>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          disabled={!isEditing}
                        />
                        <Label htmlFor="has_optical_drive" className="font-normal">
                          Оптический привод
                        </Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          disabled={!isEditing}
                        />
                        <Label htmlFor="has_card_reader" className="font-normal">
                          Картридер
                        </Label>
                      </div>
                    </div>
                    ...
                    {isEditing ? (
                      ...
                      <div className="text-sm">
                        {equipment.current_owner_name ? (
                          <div>
                            {equipment.current_owner_rank && `${equipment.current_owner_rank} `}
                            {equipment.current_owner_name}
                          </div>
                        ) : (
                          '—'
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-muted-foreground">
                      {equipment.seal_install_date && (
                        <div>
                          ...
                          {new Date(equipment.seal_install_date).toLocaleDateString('ru-RU')}
                        </div>
                      )}
                      {equipment.seal_check_date && (
                        <div>
                          <span className="font-medium">Последняя проверка:</span>{' '}
                          {new Date(equipment.seal_check_date).toLocaleDateString('ru-RU')}
                        </div>
                      )}
                    </div>
                  )}
                      ...
                      >
                        ...
                        {movement.document_number && (
                          <div className="mt-2 text-sm text-muted-foreground">
                            ...
                            {movement.document_date && (
                              <> от {new Date(movement.document_date).toLocaleDateString('ru-RU')}</>
                            )}
                          </div>
                        )}
                        ...
                        {movement.reason && (
                          <div className="mt-2 text-sm">
                            <span className="text-muted-foreground">Причина:</span> {movement.reason}
                          </div>
                        )}

                        {(movement.seal_number_before || movement.seal_number_after) && (
                          <div className="mt-2 text-sm text-muted-foreground">
                            Пломба: {movement.seal_number_before || '—'} → {movement.seal_number_after || '—'}
                            {movement.seal_status && ` (${movement.seal_status})`}
                          </div>
                        )}
