...
export default function BatchPhonePage() {
  ...
  const currentPhones = activeTab === 'checkin' ? issuedPhones?.items || [] : submittedPhones?.items || [];
  const isLoading = activeTab === 'checkin' ? isLoadingIssued : isLoadingSubmitted;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <h1 className="text-3xl font-bold mb-6">Массовая сдача/выдача телефонов</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <Card>
              <CardContent className="pt-6">
                ...
                <div className="text-sm text-muted-foreground">Всего телефонов</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-green-600">{statusReport.checked_in}</div>
                <div className="text-sm text-muted-foreground">Сдано</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-orange-600">{statusReport.checked_out}</div>
        ...
        <Card>
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={(val) => { setActiveTab(val); setSelectedPhones([]); }}>
              <TabsList className="grid w-full grid-cols-2 mb-6">
                <TabsTrigger value="checkin">
                  Вечерняя сдача
                  {notSubmittedCount > 0 && (
                    <Badge variant="destructive" className="ml-2">{notSubmittedCount}</Badge>
                  )}
                </TabsTrigger>
                ...
                {isLoading ? (
                  ...
                  <>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      >
                        {selectedPhones.length === currentPhones.length ? 'Снять всё' : 'Выбрать всё'}
                      </Button>
                      <span className="text-sm text-muted-foreground">
                        Выбрано: {selectedPhones.length} из {currentPhones.length}
                      </span>
                    </div>
                      ...
                      {currentPhones.map((phone) => (
                        <div
                          ...
                          className={`flex items-center space-x-3 p-4 border rounded-lg cursor-pointer transition-colors ${
                            selectedPhones.includes(phone.id) ? 'bg-primary/5 border-primary' : 'hover:bg-slate-50'
                          }`}
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className="font-medium">{phone.owner_full_name}</div>
                            <div className="text-sm text-muted-foreground">
                              {phone.model || 'Модель не указана'} • {phone.storage_location || 'Ячейка не указана'}
                            </div>
                          </div>
                    ...
                    <Button
                      className="w-full"
                      size="lg"
                      disabled={selectedPhones.length === 0 || checkinMutation.isPending}
                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    <Button
                      className="w-full"
                      size="lg"
                      disabled={selectedPhones.length === 0 || checkoutMutation.isPending}
                      onClick={handleSubmit}
                    >
