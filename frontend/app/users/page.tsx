...
import { ArrowLeft, Clock, Plus, Power, Search, Settings2, ShieldCheck, Trash2, User } from "lucide-react";
...
// Все роли синхронизированы с бэкендом
const ROLE_CONFIG: Record<string, { label: string; className: string; icon?: boolean }> = {
  admin:    { label: "Администратор", className: "bg-red-500/20 text-red-400 border-red-500/30", icon: true },
  officer:  { label: "Офицер",        className: "bg-purple-500/20 text-purple-400 border-purple-500/30" },
  operator: { label: "Оператор",      className: "bg-amber-500/20 text-amber-400 border-amber-500/30" },
  user:     { label: "Пользователь",  className: "bg-blue-500/20 text-blue-400 border-blue-500/30" },
};

function RoleBadge({ role }: { role: string }) {
  const config = ROLE_CONFIG[role] ?? { label: role, className: "bg-white/10 text-muted-foreground border-white/10" };
  return (
    <Badge className={`${config.className} gap-1 px-2 py-0.5 text-[10px]`}>
      {config.icon && <ShieldCheck className="h-3 w-3" />}
      {config.label}
    </Badge>
  );
}
...
function StatusBadge({ isActive }: { isActive: boolean }) {
  return isActive ? (
    <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 px-2 py-0.5 text-[10px]">Активен</Badge>
  ) : (
    <Badge variant="outline" className="text-muted-foreground border-white/10 px-2 py-0.5 text-[10px]">Отключен</Badge>
  );
}
...
export default function UsersPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: unknown) => {
      toast.error(getApiError(err, "Ошибка при удалении"));
    },
  });
  ...
  const toggleActiveMutation = useMutation({
    ...
    onError: (err: unknown) => {
      toast.error(getApiError(err, "Ошибка при изменении статуса"));
    },
  });
                ...
                <Table>
                  ...
                  <TableBody>
                    {!data?.items.length ? (
                      ...
                      data.items.map((user) => (
                        <TableRow key={user.id} className="hover:bg-white/[0.06] border-white/5 transition-colors group">
                          ...
                          <TableCell className="px-6 py-4 text-right">
                            <div className="flex justify-end gap-2">
                              ...
                              <Button
                                size="sm"
                                variant="ghost"
                                className={`h-8 hover:bg-white/10 transition-colors px-3 ${
                                  user.is_active
