...
// Все роли должны совпадать с VALID_ROLES на бэкенде (schemas/user.py)
const ROLES = [
...
const ROLE_BADGES: Record<string, { label: string; className: string }> = {
  admin: { label: "Администратор", className: "bg-red-500/20 text-red-400 border-red-500/30" },
...
export default function EditUserPage() {
  ...
  const [form, setForm] = useState<UserUpdate>(INITIAL_FORM);
  ...
  // Сбрасываем форму при смене пользователя (изменении user.id)
  useEffect(() => {
    if (user) {
      setForm({
      ...
      // Сбрасываем секцию смены пароля при переходе между пользователями
  ...
  const roleBadge = ROLE_BADGES[user.role] ?? { label: user.role, className: "bg-white/10 text-muted-foreground border-white/10" };
        ...
        <div className="mb-6">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-col items-end gap-2 shrink-0">
              <Badge className={`${roleBadge.className} gap-1`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button
                  type="submit"
                  disabled={isPending}
                  className="flex-1 gradient-primary border-0 shadow-lg gap-2"
                >
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none ${
                  changePassword ? "bg-primary" : "bg-white/20"
                }`}
              >
            ...
            <CardContent className="pt-5">
              <div className="space-y-4">
                ...
                <Button
                  ...
                  className="w-full gradient-primary border-0 shadow-lg gap-2"
                >
                  {passwordMutation.isPending
                    ? <><Loader2 className="h-4 w-4 animate-spin" />Сохранение...</>
                    : <><KeyRound className="h-4 w-4" />Установить новый пароль</>}
                </Button>
              </div>
            </CardContent>
          )}
