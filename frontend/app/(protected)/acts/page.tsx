...
/** Переводит первую букву строки в нижний регистр (только кириллица). */
...
/** «Иванов Иван Иванович» → «И. Иванов» (для строки подписи в Приложении). */
function getPersonLastNameInitials(fullName: string): string {
  const parts = fullName.trim().split(/\s+/);
  return parts.length >= 2 ? `${parts[1]?.[0] ?? ""}. ${parts[0]}` : fullName;
}

/** Формирует строку участника акта: «должность/звание Фамилия И.О.» */
...
function MultiAssetSelect({
  ...
  // Всегда показываем на один слот больше последнего выбранного — для добавления нового.
...
function DocPreview({ data }: { data: PreviewData }) {
  ...
  // Пункты 2 и 3 с фиксированными номерами:
  // для акта сдачи показываем только выбранные; для выдачи — всегда оба (с заглушкой).
  const isSdacha = actType === "sdacha";

  const passItem = passNumbers ? (
    <div style={{ marginLeft: "1.25cm" }}>2. Электронный пропуск № {passNumbers}</div>
  ) : !isSdacha ? (
    <div style={{ marginLeft: "1.25cm" }}>2. Электронный пропуск №</div>
  ) : null;

  const flashItem = flashDriveNumbers ? (
    <div style={{ marginLeft: "1.25cm" }}>3. USB-накопитель МО РФ № {flashDriveNumbers}</div>
  ) : !isSdacha ? (
    <div style={{ marginLeft: "1.25cm" }}>3. USB-накопитель МО РФ №</div>
  ) : null;

  // Текст пункта 1 в зависимости от типа акта и наличия дефектов
  let item1Suffix = ".";
  if (defects) {
    item1Suffix = ` — в неисправном состоянии, а именно: ${defects} (подробнее см. Приложение).`;
  } else if (actType === "vydacha") {
    item1Suffix = " — в исправном состоянии.";
  }

  const actionLabel = actType === "sdacha" ? "Сдал(-а):" : "Выдал(-а):";
  const actionName =
    actType === "sdacha" ? data.surrendererName || "___________" : data.issuerName || "___________";
    ...
    <div className="bg-white rounded-lg shadow-xl overflow-auto max-h-[600px]">
      ...
      >
        ...
        <div style={{ textAlign: "center", fontWeight: "bold", lineHeight: "1.4" }}>
          АКТ
          <br />
          приёма-передачи в эксплуатацию
          <br />
          оборудования
        </div>
        ...
        <div style={{ textIndent: "1.25cm", textAlign: "justify" }}>
          {actType === "sdacha" ? (
            <>
              ...
              <strong>{receiverLabel || "___________"}</strong> принял(-а), а{" "}
              <strong>{surrendererLabel || "___________"}</strong> сдал(-а) нижеперечисленное имущество:
            </>
          ) : (
            <>
              Настоящий акт составлен в том, что{" "}
              <strong>{receiverLabel || "___________"}</strong> принял(-а), а{" "}
              <strong>{issuerLabel || "___________"}</strong> выдал(-а) нижеперечисленное имущество:
            </>
          )}
        </div>
        ...
        <div style={{ textIndent: "1.25cm", textAlign: "justify" }}>
          ...
          в комплекте: {kit.length > 0 ? kit.join(", ") : "___________"}){item1Suffix}
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <div>
            {actionLabel}{" "}
            <u>{actionName}</u> /_______________
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <div>
            Принял(-а): <u>{data.receiverName || "___________"}</u> /_______________
          </div>
          <div>«___»___________ {year || "____"} г.</div>
        </div>

        {/* Приложение (только при наличии дефектов) */}
        {defects && (
          <>
            <div
              style={{
            ...
            <div style={{ textIndent: "1.25cm", marginTop: "8px" }}>
              Неисправности ноутбука «{model}», серийный номер {serial}: {defects}.
            </div>
            <div style={{ marginTop: "16px" }}>Корректность сведений подтверждаю:</div>
            <div style={{ marginTop: "8px" }}>
              {data.receiverRankShort || "рядовой"}
            </div>
            <div style={{ textAlign: "right" }}>
              {data.receiverLastNameInitials || "___________"}
            </div>
            <div>«___» ___________ {year || "____"} г.</div>
          </>
        )}
...
export default function ActsPage() {
  ...
  // При выборе оборудования из базы — подставляем производителя + модель.
  ...
  // Проверка готовности формы к генерации
  const canGenerate =
    effectiveModel.trim() !== "" &&
  ...
  const handleGenerate = () => {
    ...
    startTransition(async () => {
      try {
        ...
        // Имя файла по фамилии сдающего или получателя
        const lastName =
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {KIT_OPTIONS.map((item) => (
                  <CheckboxField
                    ...
                    onChange={(checked) =>
                      setField("kit")(
                        checked ? [...form.kit, item] : form.kit.filter((k) => k !== item),
                      )
                    }
                  />
                ))}
            ...
            <div className={sectionCls}>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="царапины на корпусе, трещина на крышке экрана"
                  rows={2}
            ...
            {!canGenerate && (
              <p className="text-xs text-slate-600 text-center">
                Заполните все обязательные поля для генерации документа
              </p>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-900/40 border border-slate-800/40 rounded-xl p-4">
              <p className="text-xs text-slate-500 leading-relaxed">
                ...
                дефектов к акту автоматически добавляется Приложение.
              </p>
            </div>
          </div>
        </div>
