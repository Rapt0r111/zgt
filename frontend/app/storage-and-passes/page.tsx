...
import { Plus, Search, ArrowLeft, UsbIcon, CreditCard, Trash2, UserPlus, UserMinus } from 'lucide-react';
...
export default function StorageAndPassesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => storageAndPassesApi.getList({
                  ...
                  {data?.items.length === 0 ? (
                    ...
                    data?.items.map((asset) => (
                      <TableRow key={asset.id}>
                        ...
                        <TableCell className="font-mono font-medium">{asset.serial_number}</TableCell>
                        <TableCell>
                          <div className="text-sm">
                            {asset.manufacturer && <div className="font-semibold">{asset.manufacturer}</div>}
                          ...
                          {asset.assigned_to_name ? (
                            <div className="text-sm">
                              ...
                              {asset.assigned_to_rank && (
                                <div className="text-muted-foreground text-xs">{asset.assigned_to_rank}</div>
                              )}
                            </div>
                          ) : (
                            '—'
