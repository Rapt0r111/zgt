...
class EquipmentService:
    ...
    def get_movement_history(self, equipment_id: int, skip: int = 0, limit: int = 50) -> tuple[List[EquipmentMovement], int]:
...
class StorageDeviceService:
    ...
    def get_by_id(self, device_id: int) -> Optional[StorageDevice]:
        return self.db.query(StorageDevice).options(
            joinedload(StorageDevice.equipment)
        ).filter(StorageDevice.id == device_id, StorageDevice.is_active == True).first()
